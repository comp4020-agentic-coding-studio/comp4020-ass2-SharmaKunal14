...
spec:
  - your conclusion is scoped to the stated condition and seeds, not a general claim
  - your retention table records whether a run that reached zero ever recovered
  - you name one property of a real trained model this six-category toy has no analogue for
---
...
<FiveMinuteCheck sessionSlug="09-recursion">
...
If you want to see a chain re-copy from its own last output before you run
the fully recursive condition on the real distribution, try the
[chain simulator](/simulator/?from=09-recursion#chain-simulator). Read it as
an analogy only: it corrupts individual characters in ordinary text, not a
six-category frequency distribution, so it cannot stand in for this week's
actual model — it can only build intuition for what re-estimating from your
own last sample does over many rounds.

The [resampling simulator](/simulator/?from=09-recursion#resampling-simulator)
is the closer analogy: it runs the same three conditions — baseline,
fully recursive, retained-data — over an editable category distribution, so
you can watch a rare category's share drift or vanish across rounds. It is
still a separate, editable illustration, not this week's own dataset or
seeded draws.
