...
related:
  ...
  - sessions/06-stemma
spec:
  - your rate-by-rate reading records the seed-to-seed spread, not only the mean
  - you name one conclusion the model's assumptions do not support
  - your trend prediction is written before you open results.csv or plot.svg
---
...
<FiveMinuteCheck sessionSlug="07-ancestry">
...
If you want a feel for compounding error before you read `settings.json`'s
numbers cold, try the
[chain simulator](/simulator/?from=07-ancestry#chain-simulator). Read it as
an analogy only: it corrupts individual characters in ordinary text, not the
four-symbol alphabet this week's synthetic ancestor is built from, so it
cannot stand in for the actual simulation — it can only build intuition for
what "copying a copy" does over many generations.
